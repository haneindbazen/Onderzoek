...
public class Test {
	...
	static final SimpleHtmlSerializer htmlSerializer =  new SimpleHtmlSerializer(props);
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			homePrototypesDir = new File(Test.class.getResource("/interfaces").getPath() +"/"+prototypesDir.getName());
		}
...
//		System.out.println("copyed to: " + homePrototypesDir.getPath());
		...
		File screenDir = new File(homePrototypesDir.getPath()+"/review/screens");
		Collection<File> screens = FileUtils.listFiles(screenDir, TrueFileFilter.TRUE, TrueFileFilter.TRUE);
		...
		for (File screen : screenList){
			...
			System.out.println("----------------");
			TagNode screenNode =  node.findElementByAttValue("class" , "screen firer ie-background commentable", true, true);
			String screenName = screenNode.getAttributeByName("name");
			File newDest = new File(homePrototypesDir.getPath()+"/review/screens/"+screenName+".html");
